...
def simulate_slider_nozzle_control(wn, zeta, Kp, Kd, target_pos, shaper_wn, shaper_zeta, time_max=0.1, dt=0.001):
    """
...
frequency = 60.0  # 想定される固有振動数 (Hz)
wn_real = frequency * 2 * np.pi  # 固有振動数をrad/sに変換
zeta_real = 0.05 
...
frequency_shaper = 70.0  # 想定される固有振動数 (Hz)
shaper_wn_setting = frequency_shaper * 2 * np.pi  # 固有振動数をrad/sに変換
...
print(len(t_train), len(xs_train), len(xn_train))
print("サンプルデータ数")

...
class WalkVecEnv(VecEnv):
    def __init__(self, num_envs, urdf_path, headless=False):
        # Action: 8 joint positions (4 legs * 2 joints)
        self.num_joints = 8
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(self.num_joints,), dtype=np.float32)
        
        # Observation: 
        # Base Pos (3) + Base Quat (4) + Base Lin Vel (3) + Base Ang Vel (3) + Joint Pos (8) + Joint Vel (8) + Foot Contact (4) + Proj Gravity (3) + Phase (2) = 38
        ...
        self.default_dof_pos = torch.tensor(
            [0.0] * 8, device="cuda:0", dtype=torch.float32
        )

        self.default_kp = 1000.0
        self.default_kd = 40.0 
    ...
    def _setup_world(self):
        ...
        for i in range(1, 5):
            foot_path = f"{template_path}/quadruped_ganimata_robot/leg{i}_lower"
            foot_prim = self.world.stage.GetPrimAtPath(foot_path)
            UsdShade.MaterialBindingAPI(foot_prim).Bind(material, UsdShade.Tokens.strongerThanDescendants)
        ...
        # 3. Create Views
        self.robots = ArticulationView(prim_paths_expr="/World/envs/env_*/quadruped_ganimata_robot", name="robots_view")
        self.world.scene.add(self.robots)
        
        self.feet = RigidPrimView(prim_paths_expr="/World/envs/env_*/quadruped_ganimata_robot/leg*_lower", name="feet_view", reset_xform_properties=False, track_contact_forces=True)
    ...
    def _import_urdf(self, prim_path):
        ...
        if result[0]:
            omni.kit.commands.execute("MovePrim", path_from="/quadruped_ganimata_robot", path_to=f"{prim_path}/quadruped_ganimata_robot")
        else:
            print("URDF Import failed")
        
    def step_async(self, actions):
        # actions: [joint_pos_targets(6)] 
    ...
    def step_wait(self):
        ...
        # Apply random force to base every N steps
        if self.step_counts[0] % self.push_interval == 0:
             # Random force in XY plane
             forces = torch.zeros((self.num_envs, 3), device="cuda:0")
             forces[:, :2] = (torch.rand((self.num_envs, 2), device="cuda:0") - 0.5) * 200.0 # +/- 100N
             # Apply to root (indices)
             # Note: apply_forces takes (N, 3) or (N, body, 3). Here we apply to root.
             # We need to get rigid body view for root? ArticulationView.apply_forces applies to bodies.
             # Assuming body 0 is root.
             # self.robots.apply_forces(forces, indices=torch.arange(self.num_envs, device="cuda:0"))
             # For simplicity, let's skip explicit push for now or use set_velocities to simulate impact
             # Simulating impact by adding velocity is more stable
             root_vels = self.robots.get_velocities()
             root_vels[:, :2] += (torch.rand((self.num_envs, 2), device="cuda:0") - 0.5) * 1.0 # +/- 0.5 m/s jump
        ...
        # Update History
        # Shift buffer: [oldest, ..., newest]
        # We want [frame_t-2, frame_t-1, frame_t]
        # Shift left by single_obs_dim
    ...
    def _compute_rewards(self):
        ...
        # Increased penalty 2.0 -> 5.0
        # projected_gravity should be (0,0,-1)
        rewards -= 5.0 * torch.sum(torch.square(projected_gravity[:, :2]), dim=1)
        
        # 5. Survival Reward (New)
        # Encourage staying alive (not falling)
        rewards += 0.1
        
        # 6. Foot Clearance (New)
        # Reward feet for being in the air (swinging)
        # We can use foot height.
        # Calculate foot positions
        joint_pos = self.robots.get_joint_positions()
        # Note: We need FK to get foot positions accurately, but we approximated it in step_wait.
        # Let's re-calculate or approximate.
        # For now, let's use a simpler proxy: Joint Velocity?
        # Or just reward lifting legs?
        # Let's use the contact forces. If not in contact, reward.
        forces = self.feet.get_net_contact_forces()
        if forces is None: forces = torch.zeros((self.num_envs, 4, 3), device="cuda:0")
        else: forces = forces.view(self.num_envs, 4, 3)
        in_contact = (torch.norm(forces, dim=2) > 0.01).float()
        
        # Reward for having feet in air (1 - in_contact)
        # But we don't want all feet in air (jumping).
        # We want roughly 2 feet in air.
        num_feet_in_air = 4.0 - torch.sum(in_contact, dim=1)
        # Reward if 1 or 2 feet are in air?
        # Let's just give a small bonus for each foot in air to encourage lifting.
    ...
    def _reset_idx(self, env_ids):
        ...
        j_pos = (torch.rand((num_resets, self.num_joints), device="cuda:0") * 0.2) - 0.1
        ...
        # Reset History
        # Fill history with current obs
        obs_single = self._compute_obs()
        # We need to extract only the reset envs obs.
        # But _compute_obs returns all.
        # Optimization: Just zero out history for reset envs and fill last frame?
        # Or fill all frames with current state (stationary assumption)
        # obs_single is (N, 46).
        # We want to set self.obs_history[env_ids] = [obs_single[env_ids], obs_single[env_ids], obs_single[env_ids]]
        
        reset_obs = obs_single[env_ids] # (K, 46)
        # Repeat 3 times
        reset_history = reset_obs.repeat(1, self.history_len) # (K, 138)

...
for i in range(num_envs):
    ...
    drive_x.CreateStiffnessAttr(800.0); drive_x.CreateDampingAttr(100.0)
    ...
    drive_y.CreateStiffnessAttr(800.0); drive_y.CreateDampingAttr(100.0)
...
for episode in range(num_episodes):
    ...
    while simulation_app.is_running():
        ...
        if world.is_playing():
            ...
            target_x = torch.tensor([target_paths_x[i][min(current_step, path_lengths[i]-1)] for i in range(num_envs)], device=device)
            target_y = torch.tensor([target_paths_y[i][min(current_step, path_lengths[i]-1)] for i in range(num_envs)], device=device)
            ...
            pos, _ = cuboids_view.get_world_poses()
            base_history.append((pos[:, :2] - offsets_tensor).clone())
            current_step += 1
    ...
    if len(base_history) > 0:
        ...
        print(f"Episode {episode} saved. Shape: {dataset_xy.shape}")

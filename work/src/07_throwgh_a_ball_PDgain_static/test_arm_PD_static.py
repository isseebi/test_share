...
# 学習済みモデルのパス
# ※注意: アクション空間が変更されたため、古いモデル(9次元出力)を使用すると次元不一致になる可能性があります。
# 新しい環境設定で学習したモデルを指定してください。
MODEL_PATH = "arm_model_50000000_steps"
...
class ArmThrowVecEnv(VecEnv):
    def __init__(self, num_envs, urdf_path, headless=False):
        # Action: 3 joint positions (Kp, Kdは除外)
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(3,), dtype=np.float32)
        
        # Observation: 3 joint pos + 3 joint vel + 3 ball pos = 9 (変更なし)
...
def main():
    try:
        ...
        if len(robot_pos_history) > 0:
            ...
            # --- 修正: アクション履歴のプロット ---
            # Kp, Kdのプロットは削除し、Position Targetのみを表示します
            plt.figure(figsize=(12, 6))
            cmd_hist = np.array(command_history)
            # cmd_histは (Steps, 3) の形状になっています
            plt.plot(cmd_hist[:, 0], label='Joint 1 Target', linestyle='-')
            plt.plot(cmd_hist[:, 1], label='Joint 2 Target', linestyle='-')
            plt.plot(cmd_hist[:, 2], label='Joint 3 Target', linestyle='-')
            plt.title("Action History (Joint Position Targets)")
            plt.xlabel("Step")
            plt.ylabel("Normalized Action (-1 to 1)")
            plt.legend()
            plt.grid(True)
